<?php
   ...
   if(isset($valider)){
      ...
      $sel = $pdo->prepare("SELECT * FROM users WHERE email=? AND password=? LIMIT 1");
      $sel->execute(array($email, $password));
      $tab = $sel->fetchAll();
      if(count($tab) > 0){
         $_SESSION["prenomNom"] = ucfirst(strtolower($tab[0]["prenom"])) . " " . strtoupper($tab[0]["nom"]);
         $_SESSION["autoriser"] = true;
         header("location: ../appweb/ChatApp/users.php");
         exit();
      }
      else {
         $erreur = "Mauvais Eemail ou mot de passe!";
      }
   }
?>
<!DOCTYPE html>
          ...
          <div class="mydiv1">
            <form class="row g-3 needs-validation" novalidate action="" method="POST">
              ...
              <div class="col-12">
                ...
                <input type="eemail" class="form-control" name="email" id="validationCustom03"
            ...
            <a href="signup1.php"> Don't have an account?</a><br>
            <a href="../Email_Verification/forgot-password.php">Forgot password?</a>
          </div>
          <div class="mydiv2">
